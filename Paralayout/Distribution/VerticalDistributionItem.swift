...
public typealias VerticalDistribution = [VerticalDistributionItem]

extension VerticalDistribution {
    public init(@VerticalDistributionBuilder _ distribution: () -> VerticalDistribution) {
        self = distribution()
    }
}
...
/// An element of a vertical distribution.
@MainActor
public enum VerticalDistributionItem: Sendable {

    /// A UIView, with adjustments to how much space it should take up.
    case view(Alignable, orthogonalAlignment: HorizontalDistributionAlignment?)
    ...
    internal static func items(
        ...
        superview: UIView?
    ) -> (items: [VerticalDistributionItem], totalFixedSpace: CGFloat, flexibleSpaceDenominator: CGFloat) {
    ...
    /// Returns the length of the distribution item along the vertical axis, applying the `multiplier` to the weight of flexible spacers and proxies.
...
extension Alignable {
    ...
    /// Specifies the horizontal alignment to use for this item in a vertical distribution, overriding the orthogonal alignment specified for the distribution as a whole.
    @MainActor
